...
func TestParseConfig(t *testing.T) {
	cases := map[string]struct {
		...
		"defaults": {
			expectedCfg: &Config{8080, &config{Upstreams: map[string]*cfgUpstream{"front": &cfgUpstream{Balancer: "round_robin", Provider: "static", Servers: []upstreamServer{upstreamServer{Path: "http://api.foo.com", Weight: 5}}}, "backend": &cfgUpstream{Balancer: "round_robin", Provider: "static", Servers: []upstreamServer{upstreamServer{Path: "http://api.foo1.com", Weight: 5}, upstreamServer{Path: "http://api.foo2.com", Weight: 0}}}}, Server: &server{Port: 8080, Locations: []serverLocation{serverLocation{RegEx: "/api", Upstream: "backend"}, serverLocation{RegEx: "/", Upstream: "front"}}}}},
			expectedErr: nil,
		},
		"valid": {
			expectedCfg: &Config{80, &config{Upstreams: map[string]*cfgUpstream{"front": &cfgUpstream{Balancer: "round_robin", Provider: "static", Servers: []upstreamServer{upstreamServer{Path: "http://api.foo.com", Weight: 5}}}, "backend": &cfgUpstream{Balancer: "round_robin", Provider: "static", Servers: []upstreamServer{upstreamServer{Path: "http://api.foo1.com", Weight: 5}, upstreamServer{Path: "http://api.foo2.com", Weight: 0}}}}, Server: &server{Port: 80, Locations: []serverLocation{serverLocation{RegEx: "/api", Upstream: "backend"}, serverLocation{RegEx: "/", Upstream: "front"}}}}},
			expectedErr: nil,
		},
	}
